...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d, load=True, decode_init=1, losses="loss1", lambda_mask=0.75):
        ...
        if cfg.decoder == 'MLPDecoder':
            ...
            from .decoders.MLPDecoder import DecoderHead3
            self.decode_head = DecoderHead3(in_channels=self.channels, num_classes=cfg.num_classes, norm_layer=norm_layer, embed_dim=cfg.decoder_embed_dim, losses=losses, lambda_mgd=lambda_mask)
