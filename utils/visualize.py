...
def print_iou(iou, freq_IoU, mean_pixel_acc, pixel_acc, class_names=None, show_no_back=False, no_print=False):
    ...
    #     print('mean_IoU: %.3f%%, freq_IoU: %.3f%%, mean_pixel_acc: %.3f%%, pixel_acc: %.3f%%' % (mean_IoU * 100, freq_IoU * 100, mean_pixel_acc * 100, pixel_acc * 100))

    # else:
    #     # lines.append('----------     %-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%' % ('mean_IoU', mean_IoU * 100, 'freq_IoU', freq_IoU*100, 'mean_pixel_acc', mean_pixel_acc*100, 'pixel_acc',pixel_acc*100))
    #     print('mean_IoU: %.3f%%, freq_IoU: %.3f%%, mean_pixel_acc: %.3f%%, pixel_acc: %.3f%%' % (mean_IoU * 100, freq_IoU * 100, mean_pixel_acc * 100, pixel_acc * 100))

    # line = "\n".join(lines)
    
    return line, mean_IoU * 100
